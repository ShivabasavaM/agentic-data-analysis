...
function App() {
  ...
  const [uploadedFileId, setUploadedFileId] = useState(null); // Tracks backend UUID
  const [uploadStatus, setUploadStatus] = useState(''); // Tracks upload UI state
  ...
  // NEW: Secure file upload handler to get the UUID from FastAPI
  const handleFileUpload = async (selectedFile) => {
    ...
    try {
      ...
      setUploadedFileId(data.file_id); // Save the strict UUID generated by the backend
      setUploadStatus('File uploaded successfully!');
    } catch (error) {
      console.error(error);
      setUploadStatus('Error uploading file. Check backend connection.');
    }
  };
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '40px auto', fontFamily: 'Arial, sans-serif', color: '#333', position: 'relative' }}>
      ...
      <div style={{ background: '#f4f4f9', padding: '15px', borderRadius: '8px', marginBottom: '20px', display: 'flex', flexDirection: 'column', gap: '10px', border: '1px solid #e0e0e0' }}>
        <div style={{ display: 'flex', gap: '20px', alignItems: 'center', flexWrap: 'wrap' }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: '5px', cursor: 'pointer' }}>
            <input type="radio" name="dataSource" value="default" checked={dataSource === 'default'} onChange={() => { setDataSource('default'); setUploadStatus(''); }} />
            Use Default Data
          </label>
        ...
        {dataSource === 'custom' && (
          <div style={{ marginTop: '5px', padding: '10px', background: '#fff', border: '1px dashed #ccc', borderRadius: '4px' }}>
            {/* FIX: Trigger the handler on change to actually upload the file to backend */}
            <input type="file" accept=".csv" onChange={(e) => handleFileUpload(e.target.files[0])} style={{ fontSize: '13px', marginBottom: '8px' }} />
            
            {uploadStatus && (
              <div style={{ fontSize: '12px', fontWeight: 'bold', color: uploadStatus.includes('Error') ? '#d32f2f' : '#2e7d32', marginBottom: '5px' }}>
                {uploadStatus}
              </div>
            )}
      ...
      <div style={{ display: 'flex', gap: '10px' }}>
        ...
        <button 
          onClick={handleAsk} 
          disabled={loading || (dataSource === 'custom' && !uploadedFileId) || !input.trim()} 
          style={{ 
            padding: '0 25px', 
            background: (loading || (dataSource === 'custom' && !uploadedFileId) || !input.trim()) ? '#ccc' : '#a50050',
            color: '#fff', 
            border: 'none', 
            borderRadius: '4px', 
            cursor: (loading || (dataSource === 'custom' && !uploadedFileId) || !input.trim()) ? 'not-allowed' : 'pointer',
            fontWeight: 'bold',
